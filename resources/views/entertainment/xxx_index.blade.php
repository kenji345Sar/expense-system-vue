...
<div class="max-w-6xl mx-auto py-8">
    <h1 class="text-2xl font-bold mb-6">接待交際費一覧</h1>

    <x-alert-message />

    <div class="mb-4">
        <x-action-button :href="route('entertainment.create')" color="blue" class="text-xl text-black-800 font-semibold">
            ＋ 新規申請
        </x-action-button>
    </div>
    ...
    <table class="min-w-full bg-white border border-gray-300">
        <thead class="bg-gray-100">
            <tr>
                ...
                @endif
                <x-table.th>利用日</x-table.th>
                <x-table.th>接待相手</x-table.th>
                <x-table.th>場所</x-table.th>
                <x-table.th>内容</x-table.th>
                <x-table.th>金額</x-table.th>
        ...
        <tbody>
            ...
            @php
            $details = $expense->entertainmentExpenses;
            @endphp
            ...
            <tr>
                ...
                <x-table.td>{{ $detail->entertainment_date }}</x-table.td>
                <x-table.td>{{ $detail->client_name }}</x-table.td>
                <x-table.td>{{ $detail->place }}</x-table.td>
                <x-table.td>{{ $detail->content }}</x-table.td>
                <x-table.td>{{ number_format($expense->amount) }} 円</x-table.td>
                @else
                <td class="p-2 border">明細 {{ $details->count() }} 件</td>
                <td class="p-2 border" colspan="3">詳細は別途</td>
                ...
                <x-table.td>
                    <div class="flex flex-wrap gap-x-4 gap-2">
                        <x-action-button type="form"
                            method="GET"
                            :href="route('entertainment.edit', $expense->id)"
                            color="green"
                            formMargin="mr-[6px]"
                            class="text-sm">
                            編集
                        ...
                        <x-action-button type="form" :href="route('entertainment.submit', $expense->id)" method="POST" color="blue" class="text-sm">
                            申請
                        </x-action-button>

                        @if (auth()->user()->is_approver && $expense->status === 'submitted')
                        <x-action-button type="form" :href="route('entertainment.approve', $expense->id)" method="POST" color="purple">
                            承認
                        </x-action-button>
                        @endif
                    </div>
